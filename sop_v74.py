...
def classify_regime(vix, atr, atr_median):
    if vix > 18 or atr > 1.5 * atr_median: return "High_Volatility"
    if vix < 13 or atr < 0.9 * atr_median: return "Low_Volatility"
    return "Normal_Market"

def thresholds(regime, vix):
    if regime == "High_Volatility": bull, bear = 2.3, 3.0
    elif regime == "Low_Volatility": bull, bear = 1.7, 2.5
    else: bull, bear = 1.5, 2.5
    if vix < 11: bull = max(1.2, bull - 0.3)
    return bull, bear

def htf_trend(htf_bars):
    if len(htf_bars) < 2: return None
    if htf_bars[-1]['close'] > htf_bars[-2]['close']: return 'up'
    if htf_bars[-1]['close'] < htf_bars[-2]['close']: return 'down'
    return None
...
def inside_break(prev_h, prev_l, h, l, o2, c2):
    if h <= prev_h and l >= prev_l:
        if abs(c2-o2) > 0.6*(h-l): return True
    return False
